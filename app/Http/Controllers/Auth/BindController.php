...
class BindController extends BaseController
{
    ...
    public function checkAccount(StoreRequest $request){
        $count = User::where('account','=',HttpFilter($request->input('account')))->count();
        if($count==0){
    ...
    public function checkUserLogin(StoreRequest $request){
        ...
        if($count>0){
            ...
            //获取到用户信息
            $user = User::where('account','=',HttpFilter($account))->first();
            if(!App::make('hash')->check(HttpFilter($password),$user['password'])){
    ...
    public function bindCode(StoreRequest $request){
        ...
        $mobile = HttpFilter($request->input('mobile'));
        //获取验证码
        $code = HttpFilter($request->input('code'));
        //从redis中获取验证码
        ...
        if($codes==null){
            ...
            //生成二维码
            $qrCodeUrl = $google2fa->getQRCodeUrl("EPayPlusBusiness",$mobile,$user['ggkey']);
